...
public sealed class SqlServerPurchaseOrdersIntegrationTests
{
    ...
    public async Task SqlServerPurchaseOrderRepository_CreatesUpdatesReadsAndDeletesAnOrder()
    {
        ...
        try
        {
            ...
            var created = await repository.GetAsync(CompanyCode, number, CancellationToken.None);
            ...
            var updated = await repository.GetAsync(CompanyCode, number, CancellationToken.None);
            Assert.NotNull(updated); Assert.Equal(600m, updated.Lines[0].AM_SUPPLY);
            Assert.True(await repository.DeleteAsync(CompanyCode, number, CancellationToken.None));
            Assert.Null(await repository.GetAsync(CompanyCode, number, CancellationToken.None));
        }
        ...
        {
            await CleanupAndAssertNoResidueAsync(repository, number, testFailure);
        }
    }
    ...
    public async Task SqlServerPurchaseOrderApi_ValidatesAndRecalculatesAmounts()
    {
        ...
        var cleanupRepository = new SqlServerPurchaseOrderRepository(new SqlServerConnectionFactory(ConnectionString));
        Exception? testFailure = null;
        ...
        try
        {
            ...
            Assert.Equal(HttpStatusCode.NoContent, (await client.DeleteAsync($"/api/purchase-orders/{CompanyCode}/{request.Header.NO_PO}")).StatusCode);
        }
        catch (Exception exception)
        {
            testFailure = exception;
        ...
        {
            await CleanupAndAssertNoResidueAsync(cleanupRepository, request.Header.NO_PO, testFailure);
        }
    }
    ...
    private static async Task AssertNoResidueAsync(string purchaseOrderNo)
    {
        ...
        command.CommandText = """
            SELECT
                (SELECT COUNT(*) FROM POC.PUR_POL WHERE CD_FIRM = @firm AND NO_PO = @purchaseOrderNo) +
                (SELECT COUNT(*) FROM POC.PUR_POH WHERE CD_FIRM = @firm AND NO_PO = @purchaseOrderNo);
            """;
