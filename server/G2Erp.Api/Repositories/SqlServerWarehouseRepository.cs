...
public sealed class SqlServerWarehouseRepository(SqlServerConnectionFactory connections) : IWarehouseRepository
{
    ...
    public async Task AddAsync(Warehouse warehouse, CancellationToken ct) { const string sql="INSERT INTO POC.MA_WH(CD_FIRM,CD_WH,NM_WH,YN_USE) VALUES(@firm,@warehouse,@name,@useYn)"; await using var c=connections.Create(); await c.OpenAsync(ct); await using var cmd=new SqlCommand(sql,c); cmd.Parameters.AddWithValue("@firm",warehouse.CD_FIRM);cmd.Parameters.AddWithValue("@warehouse",warehouse.CD_WH);cmd.Parameters.AddWithValue("@name",warehouse.NM_WH);cmd.Parameters.AddWithValue("@useYn",warehouse.YN_USE);await cmd.ExecuteNonQueryAsync(ct); }
    public async Task<bool> DeleteAsync(string companyCode, string warehouseCode, CancellationToken ct) { await using var c=connections.Create();await c.OpenAsync(ct);await using var cmd=new SqlCommand("DELETE FROM POC.MA_WH WHERE CD_FIRM=@firm AND CD_WH=@warehouse",c);cmd.Parameters.AddWithValue("@firm",companyCode);cmd.Parameters.AddWithValue("@warehouse",warehouseCode);return await cmd.ExecuteNonQueryAsync(ct)>0; }
