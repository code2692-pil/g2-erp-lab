...
export function ReviewPackageImportPanel() {
  ...
  const [status, setStatus] = useState("검토 패키지 JSON을 선택하면 현재 분석과 분리된 검토용 요약으로 확인합니다.");
  ...
  const handleFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = "";
    if (!file) return;
    if (file.size > maximumPackageSize) { setError("검토 패키지는 최대 256KB까지 불러올 수 있습니다."); return; }
    ...
    try {
      ...
      if (!validation.success) { setError(`검토 패키지를 불러오지 않았습니다: ${validation.error}`); return; }
      setLoadedPackage(validation.value);
      setStatus("검토 패키지를 현재 분석과 분리된 요약으로 불러왔습니다.");
    } catch {
      setError("검토 패키지를 불러오지 않았습니다: UTF-8 JSON 형식을 확인해 주세요.");
  ...
  return <section className="ai-solution-center__review-package-import" data-testid="review-package-import" aria-busy={processing} aria-labelledby="review-package-import-heading">
    <div><h2 id="review-package-import-heading">검토 패키지 확인</h2><p>불러온 패키지는 현재 분석 입력이나 결과를 자동으로 변경하지 않습니다.</p></div>
    <label className="ai-solution-center__file-label" htmlFor="review-package-input"><FileJson size={16} />검토 패키지 불러오기<input id="review-package-input" data-testid="review-package-input" type="file" accept=".json,application/json" onChange={(event) => void handleFile(event)} /></label>
    ...
    {review && <section className="ai-solution-center__loaded-review" data-testid="loaded-review-package"><div className="ai-solution-center__loaded-review-heading"><div><h3>{review.caseTitle}</h3><p>{reviewStatusLabels[review.reviewStatus]} · 추천 1순위: {review.recommendedOption}</p></div><button data-testid="review-package-remove" type="button" onClick={() => { setLoadedPackage(undefined); setStatus("불러온 검토 패키지를 제거했습니다. 현재 분석은 유지됩니다."); }}><Trash2 size={15} />제거</button></div><dl><div><dt>생성 시각</dt><dd>{review.createdAt}</dd></div><div><dt>수정 시각</dt><dd>{review.updatedAt}</dd></div><div><dt>검토자 역할</dt><dd>{review.reviewerRole ? reviewerRoleLabels[review.reviewerRole] : "선택 안 함"}</dd></div><div><dt>체크리스트</dt><dd>{Object.values(review.checklist).filter(Boolean).length}개 확인</dd></div><div><dt>컨설턴트 의견</dt><dd>{review.consultantReview || "없음"}</dd></div><div><dt>개발 의견</dt><dd>{review.developerReview || "없음"}</dd></div><div><dt>결정 사유</dt><dd>{review.decisionReason || "없음"}</dd></div><div><dt>제한사항</dt><dd>{review.limitations.join(" / ")}</dd></div></dl></section>}
  </section>;
}
