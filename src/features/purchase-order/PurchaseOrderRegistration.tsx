...
export function PurchaseOrderRegistration({ onNavigate, showDevelopmentDataManager = false }: PurchaseOrderRegistrationProps) {
    ...
    const focusValidationIssue = (issue: {
        ...
        if (!issue.rowKey || !issue.field || (issue.scope !== "header" && issue.scope !== "line"))
            return;
    ...
    const guardNavigation = async (event: MouseEvent) => {
            ...
            const nextPage = button?.dataset.testid === "nav-sales-order"
                ...
                : button?.dataset.testid === "nav-work-order"
                    ? "work"
                    : button?.dataset.testid === "nav-development"
                        ? "development"
                        : null;
            if (!nextPage || !isDirty)
                return;
