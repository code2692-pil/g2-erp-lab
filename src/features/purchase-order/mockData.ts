...
const headerRows: Array<Omit<PurchaseOrderHeader, "DC_RMK"> & { DC_RMK?: string }> = [
  ...
  ,...Array.from({ length: 5 }, (_, index) => ({ CD_FIRM: "1000", NO_PO: `POR202608${String(index + 1).padStart(4, "0")}`, DT_PO: `2026-08-${String(index + 2).padStart(2, "0")}`, CD_PARTNER: index % 2 === 0 ? "SUP-001" : "SUP-002", NM_PARTNER: index % 2 === 0 ? "세림정밀" : "한빛전자", CD_EMP: "E-001", NM_EMP: "구매담당", CD_CURRENCY: "KRW", RT_EXCHANGE: 1, ST_PO: index === 3 ? "진행" : "신규", DC_RMK: `원본 SOR202608${String(index + 1).padStart(4, "0")}/1` }))
];

export const mockPurchaseOrderHeaders: PurchaseOrderHeader[] = headerRows.map((header) => ({ ...header, DC_RMK: "정기 구매" }));
...
export const mockPurchaseOrderLines: PurchaseOrderLine[] = [
  ...
  ...[24, 40, 40, 30, 50].map((quantity, index) => line("1000", `POR202608${String(index + 1).padStart(4, "0")}`, 1, "ITM-1001", "220V 서보드라이브 모듈", "SV-220V", "EA", quantity, 180000, "WH-RM-01", "자재창고", `2026-08-${String(index + 15).padStart(2, "0")}`, `원본 SOR202608${String(index + 1).padStart(4, "0")}/1`))
];
