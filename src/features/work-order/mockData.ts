...
export const mockWorkOrderHeaders: WorkOrderHeader[] = [
  ...
  ...[20, 10, 40, 25, 35].map((quantity, index): WorkOrderHeader => ({ CD_FIRM: "1000", NO_WO: `WMO202608${String(index + 1).padStart(4, "0")}`, DT_WO: `2026-08-${String(index + 2).padStart(2, "0")}`, CD_ITEM: "ITM-1001", NM_ITEM: "220V 서보드라이브 모듈", STND_ITEM: "FINAL-UAT-FG", UNIT_ITEM: "EA", QT_WO: quantity, QT_RESULT: 0, DT_PLAN_START: `2026-08-${String(index + 3).padStart(2, "0")}`, DT_PLAN_END: `2026-08-${String(index + 5).padStart(2, "0")}`, CD_LINE: "LINE-A", NM_LINE: "조립 작업장", ST_WO: index === 3 ? "진행" : "미확정", YN_URGENT: "N", DC_RMK: `FINAL-UAT-202608 · 원본 SOR202608${String(index + 1).padStart(4, "0")}/1` }))
];
