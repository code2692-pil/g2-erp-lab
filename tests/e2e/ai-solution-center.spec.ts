...
test("Gate 12-4 A: an unsupported MP4 note drives a local recommendation and input evidence", async ({ page }) => {
  ...
  await page.getByTestId("ai-consultant-analyze").click();
  await expect(page.getByTestId("ai-result")).toBeVisible();
  await expect(page.getByTestId("ai-result-phased-plan")).toContainText("첨부 1 (VIDEO) 메모");
  await expect(page.getByTestId("ai-result-phased-plan")).not.toContainText("process-video.mp4");
  await expect(page.getByTestId("input-evidence-file-1-note")).toContainText("첨부 1 (VIDEO)");
  await expect(page.getByTestId("input-evidence-file-1-note")).not.toContainText("process-video.mp4");
...
test("Gate 12-7 H: 민감정보 원문은 화면·Markdown·검토 패키지에 남지 않는다", async ({ page }) => {
  ...
  const sensitiveFileName = `secure-access_token=${token}.txt`;
  await page.getByTestId("ai-file-input").setInputFiles({ name: sensitiveFileName, mimeType: "text/plain", buffer: Buffer.from(`LOT 검사 담당 ${email} ${phone} access_token=${token}`) });
  ...
  await expect(page.getByTestId("ai-result")).not.toContainText(sensitiveFileName);
  ...
  expect(markdown).not.toContain(sensitiveFileName);
  ...
  expect(body).not.toContain(sensitiveFileName);
